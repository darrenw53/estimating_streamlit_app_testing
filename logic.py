    ...
    *,
    ...
    def _build_items(req_parts):
        ...
        for r in (req_parts or []):
            ...
            # Precompute rotated variants (0/90)
            variants = {}
            variants[0] = poly0
            if allow_rot:
                p90 = rotate(poly0, 90, origin=(0, 0), use_radians=False)
                p90 = _normalize_to_origin(p90)
                variants[90] = p90

            for _ in range(qty):
                # Use max bounds of any allowed rotation for quick shelf sizing
                bw0 = variants[0].bounds[2] - variants[0].bounds[0]
                bh0 = variants[0].bounds[3] - variants[0].bounds[1]
                if 90 in variants:
                    bw9 = variants[90].bounds[2] - variants[90].bounds[0]
                    bh9 = variants[90].bounds[3] - variants[90].bounds[1]
                    max_side = max(bw0, bh0, bw9, bh9)
                else:
                    max_side = max(bw0, bh0)
                items.append({
                    "part_name": part_name,
                    "source": source,
    ...
    def _pack_on_sheet(items, sheet_w, sheet_h, *, sort_key):
        # Usable region with edge margin
        usable = box(
            float(edge_margin),
            float(edge_margin),
            float(sheet_w) - float(edge_margin),
            float(sheet_h) - float(edge_margin),
        )
        if usable.area <= 0:
            return {"width": float(sheet_w), "height": float(sheet_h), "parts": [], "unplaced_parts": items}

        placed_polys = []  # buffered polys for collision checks
        placed_parts = []
        shelves = []  # each: y, height, x_cursor

        step = max(float(part_gap) if part_gap else 0.0, 0.25)
        ...
        for it in items_sorted:
            ...
            # Try both rotations
            rot_orders = [0]
            if allow_rot and 90 in it["variants"]:
                rot_orders = [0, 90]

            for rot in rot_orders:
                poly0 = it["variants"][rot]
                # Compute bbox dims for this rotation
                minx, miny, maxx, maxy = poly0.bounds
                bw = maxx - minx
                bh = maxy - miny

                # Try on existing shelves
                for shelf in shelves:
                    # scan x within shelf
                    x = float(edge_margin) + float(shelf["x_cursor"])
                    y = float(edge_margin) + float(shelf["y"])
                    # quick reject if doesn't fit in usable bbox
                    if (x + bw) > (float(sheet_w) - float(edge_margin) + 1e-9):
                        continue
                    if (y + bh) > (float(edge_margin) + float(shelf["y"]) + float(shelf["height"]) + 1e-9):
                        continue

                    # Try a few x nudges to resolve polygon collisions
                    x_try = x
                    while (x_try + bw) <= (float(sheet_w) - float(edge_margin) + 1e-9):
                        p = translate(poly0, xoff=x_try, yoff=y)
                        # Apply part gap by buffering half gap on each part
                        pad = float(part_gap) / 2.0 if part_gap else 0.0
                        p_chk = p.buffer(pad) if pad > 0 else p
                        if not usable.contains(p_chk):
                            x_try += step
                            continue
                        bad = False
                        for pp in placed_polys:
                            if p_chk.intersects(pp):
                                bad = True
                                break
                        if bad:
                            x_try += step
                        ...
                        # Place it
                        shelf["x_cursor"] = max(float(shelf["x_cursor"]), float(x_try - float(edge_margin)) + bw + float(part_gap))
                        placed_polys.append(p_chk)
                ...
                # New shelf attempt
                used_h = sum(float(s["height"]) for s in shelves)
                if (used_h + bh) <= (float(sheet_h) - 2.0 * float(edge_margin) + 1e-9):
                    shelf = {"y": used_h, "height": bh, "x_cursor": 0.0}
                    shelves.append(shelf)
                    x = float(edge_margin)
                    y = float(edge_margin) + float(shelf["y"])

                    # Place at left, then try nudges if collision (rare in new shelf)
                    x_try = x
                    while (x_try + bw) <= (float(sheet_w) - float(edge_margin) + 1e-9):
                        p = translate(poly0, xoff=x_try, yoff=y)
                        pad = float(part_gap) / 2.0 if part_gap else 0.0
                        ...
                        shelf["x_cursor"] = bw + float(part_gap)
                        placed_polys.append(p_chk)
                        pts = [(float(xx), float(yy)) for (xx, yy) in list(p.exterior.coords)]
                        placed_parts.append({
                            "x": float(x_try),
                    ...
                    if not placed:
                        # Could not place even on new shelf; remove shelf we just added
                        shelves.pop()
    ...
    for sort_key in sort_orders:
        for primary in clean_stock:
            ...
            # Metrics (use bounding boxes for utilization estimate to match existing behavior)
            total_stock_area = 0.0
            total_part_area = 0.0
            for sh in sheets:
                total_stock_area += float(sh["width"]) * float(sh["height"])
