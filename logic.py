...
# ============================================================
# Mixed plate nesting: DXF true-profile polygons + manual rectangles
# ============================================================

def optimize_plate_nesting_mixed(
    required_parts,
    stock_sizes,
    *,
    edge_margin=0.0,
    ...
    """
    Mixed nesting solver:
      - If a part provides 'profile_wkt', we nest its true outer polygon.
      - Otherwise we nest it as a rectangle using width/height.

    Returns same structure as optimize_plate_nesting(), but each placed part may also include:
      - 'poly': list[(x,y)] exterior points (inches) for UI rendering.
    """
    # Lazy imports (keeps app usable even if shapely isn't installed for some reason)
    try:
        ...
        # Fallback: rectangle-only nesting
        return optimize_plate_nesting(
            required_parts,
            stock_sizes,
            edge_margin=edge_margin,
            part_gap=part_gap,
    ...
    def _poly_from_wkt(wkt_str: str):
        try:
            if not wkt_str:
                return None
            p = wkt_loads(wkt_str)
            ...
            # Ensure polygon-like
            if hasattr(p, "geoms"):
                # If it's a multipolygon, keep largest
                try:
            ...
            if not getattr(p, "is_valid", True):
                try:
                    ...
                    pass
            if p is None or getattr(p, "area", 0.0) <= 0:
                return None
            return p
        except Exception:
            return None

    def _normalize_to_origin(p: Polygon) -> Polygon:
        minx, miny, maxx, maxy = p.bounds
        return translate(p, xoff=-minx, yoff=-miny)

    def _build_items(req_parts):
        """Expand required parts into a per-piece list with cached polygons + bbox dims.

        Performance note:
        The original implementation used an x-scan loop (nudging in small steps)
        which can get slow as part counts grow. This version uses a shelf packer
        driven by bounding boxes (fast), while still carrying the *true* polygon
        for accurate collision checks + UI rendering.
        """
        items = []
        for r in (req_parts or []):
            qty = _safe_int(r.get("quantity", 1), 1)
            if qty <= 0:
                continue

            part_name = str(r.get("part_name", "Part"))
            source = str(r.get("source", "") or "")

            wkt_str = str(r.get("profile_wkt", "") or "")
            poly_raw = _poly_from_wkt(wkt_str)

            if poly_raw is None:
                w = _safe_float(r.get("width", 0.0), 0.0)
                h = _safe_float(r.get("height", 0.0), 0.0)
                if w <= 0 or h <= 0:
                    continue
                poly0 = box(0, 0, w, h)
            else:
                poly0 = _normalize_to_origin(poly_raw)

            # Precompute rotated variants (0/90) and bbox dims
            variants = {0: poly0}
            if allow_rot:
                p90 = rotate(poly0, 90, origin=(0, 0), use_radians=False)
                variants[90] = _normalize_to_origin(p90)

            # Cache bbox dims per rotation for fast packing
            bbox_dims = {}
            max_side = 0.0
            for rot, p in variants.items():
                minx, miny, maxx, maxy = p.bounds
                bw = float(maxx - minx)
                bh = float(maxy - miny)
                bbox_dims[int(rot)] = (bw, bh)
                max_side = max(max_side, bw, bh)

            base_area = float(poly0.area)
                ...
                items.append(
                    {
                        "part_name": part_name,
                        "source": source,
                        "variants": variants,          # rotation -> Polygon at origin
                        "bbox_dims": bbox_dims,        # rotation -> (bw, bh)
                        "base_area": base_area,
                        "max_side": float(max_side),
                        "bbox_w0": float(bbox_dims.get(0, (0.0, 0.0))[0]),
                        "bbox_h0": float(bbox_dims.get(0, (0.0, 0.0))[1]),
                    }
                )
        return items

    def _sort_items(items, key):
        if key == "area":
            return sorted(items, key=lambda x: x["base_area"], reverse=True)
        if key == "max_side":
            return sorted(items, key=lambda x: x["max_side"], reverse=True)
        if key == "height":
            return sorted(items, key=lambda x: x["bbox_h0"], reverse=True)
        if key == "width":
            return sorted(items, key=lambda x: x["bbox_w0"], reverse=True)
        return list(items)

    def _pack_on_sheet(items, sheet_w, sheet_h, *, sort_key):
        """Fast shelf packer with polygon-carry-through.

        - Uses bbox-based shelf placement (deterministic, fast)
        - Optionally verifies polygon collision for safety using an STRtree
        """
        pad = float(part_gap) / 2.0 if part_gap else 0.0

        # Usable region with edge margin + half-gap padding.
        # This avoids expensive geometry buffering for each placement.
        inset = float(edge_margin) + float(pad)
        usable = box(
            inset,
            inset,
            float(sheet_w) - inset,
        ...
        if usable.area <= 0:
            return {"width": float(sheet_w), "height": float(sheet_h), "parts": [], "unplaced_parts": items}

        placed_parts = []
        placed_bboxes = []  # bbox rectangles for collision checks
        shelves = []  # each: y, height, x_cursor

        items_sorted = _sort_items(items, sort_key)
        unplaced = []

        def _rebuild_index():
            # Build a spatial index over placed bboxes. This keeps collision checks fast.
            if not placed_bboxes:
                return None
            try:
                return STRtree(list(placed_bboxes))
            except Exception:
                return None

        bbox_index = None

        def _try_place(poly0: Polygon, bw: float, bh: float, x: float, y: float):
            # Quick bbox containment against the inset usable bounds
            if (x + bw) > (float(sheet_w) - inset + 1e-9):
                return None
            if (y + bh) > (float(sheet_h) - inset + 1e-9):
                return None

            # BBox with inherent half-gap padding via the usable inset.
            bb = box(float(x), float(y), float(x) + float(bw), float(y) + float(bh))
            if not usable.contains(bb):
                return None

            nonlocal bbox_index
            if bbox_index is None and placed_bboxes:
                bbox_index = _rebuild_index()

            cand_idxs = []
            if bbox_index is not None:
                try:
                    cands = bbox_index.query(bb)
                    # STRtree returns geometries, map them back to indices
                    # by identity lookup (fast enough for our sizes)
                    s = set(id(g) for g in cands)
                    cand_idxs = [i for i, g in enumerate(placed_bboxes) if id(g) in s]
                except Exception:
                    cand_idxs = list(range(len(placed_bboxes)))
            else:
                cand_idxs = list(range(len(placed_bboxes)))

            for i in cand_idxs:
                try:
                    if bb.intersects(placed_bboxes[i]) and (not bb.touches(placed_bboxes[i])):
                        return None
                except Exception:
                    return None

            p = translate(poly0, xoff=float(x), yoff=float(y))
            return (p, bb)
        ...
        for it in items_sorted:
            ...
            rot_orders = [0]
            if allow_rot and 90 in it.get("variants", {}):
                rot_orders = [0, 90]

            for rot in rot_orders:
                poly0 = it["variants"][rot]
                bw, bh = it.get("bbox_dims", {}).get(int(rot), (0.0, 0.0))
                if bw <= 0 or bh <= 0:
                    continue

                # Try existing shelves: place at current cursor only (fast)
                for shelf in shelves:
                    x = float(inset) + float(shelf["x_cursor"])
                    y = float(inset) + float(shelf["y"])
                    # Must fit within this shelf's height
                    if (bh - float(shelf["height"])) > 1e-9:
                        continue

                    res = _try_place(poly0, bw, bh, x, y)
                    if res is None:
                        continue

                    p, bb = res
                    shelf["x_cursor"] = float(shelf["x_cursor"]) + float(bw) + float(part_gap)
                    placed_bboxes.append(bb)
                    bbox_index = None  # invalidate; rebuild lazily

                    pts = [(float(xx), float(yy)) for (xx, yy) in list(p.exterior.coords)]
                    placed_parts.append(
                        {
                            "x": float(x),
                            "y": float(y),
                            ...
                            "rotated": bool(rot == 90),
                            "rotation": int(rot),
                            "part_name": it.get("part_name", "Part"),
                            "source": it.get("source", ""),
                            "poly": pts,
                            "poly_area": float(getattr(poly0, "area", 0.0)),
                        }
                    )
                    placed = True
                    break
                ...
                # New shelf attempt
                # Add vertical part_gap between shelves so adjacent rows don't "touch"
                used_h = sum(float(s["height"]) for s in shelves) + float(part_gap) * float(len(shelves))
                if (used_h + bh) <= (float(sheet_h) - 2.0 * float(inset) + 1e-9):
                    shelf = {"y": used_h, "height": bh, "x_cursor": 0.0}
                    shelves.append(shelf)
                    x = float(inset)
                    y = float(inset) + float(shelf["y"])
                    ...
                    res = _try_place(poly0, bw, bh, x, y)
                    if res is None:
                        # Remove shelf we just added
                        shelves.pop()
                        continue

                    p, bb = res
                    shelf["x_cursor"] = float(bw) + float(part_gap)
                    placed_bboxes.append(bb)
                    bbox_index = None
        ...
        return {"width": float(sheet_w), "height": float(sheet_h), "parts": placed_parts, "unplaced_parts": unplaced}

    # Expand required parts into per-piece items
    items = _build_items(required_parts)
    if not items:
        return {"best": None, "candidates": []}
    ...
    # Heuristic sweeps can be expensive for large part counts.
    # For min_sheets we usually get the best result with an area-based ordering.
    if str(objective) == "max_utilization":
        sort_orders = ["area", "max_side"]
    else:
        sort_orders = ["area"]
    ...
    for sort_key in sort_orders:
        for primary in clean_stock:
            ...
            # If a particular stock size cannot place even a single part,
            # try other sizes before giving up (prevents 0-sheet results).
            rejected_sizes = set()
            ...
            while remaining:
                ...
                key_sz = (round(float(sheet_size["width"]), 6), round(float(sheet_size["height"]), 6))
                if key_sz in rejected_sizes:
                    continue

                layout = _pack_on_sheet(
                    remaining,
                    sheet_size["width"],
                    sheet_size["height"],
                    sort_key=sort_key,
                ...
                placed = layout.get("parts", [])
                if not placed:
                    # This sheet size can't place the *current* remaining set; reject it and try another.
                    rejected_sizes.add(key_sz)
                    # If we've rejected all available sizes, bail.
                    if len(rejected_sizes) >= len(clean_stock):
                        break
                    continue

                sheets.append(layout)
                remaining = [dict(it) for it in layout.get("unplaced_parts", [])]
            ...
            for sh in sheets:
                total_stock_area += float(sh["width"]) * float(sh["height"])
                for p in sh.get("parts", []):
                    # Prefer true polygon area when available
                    pa = p.get("poly_area", None)
                    if pa is not None and float(pa) > 0:
                        total_part_area += float(pa)
                    else:
                        total_part_area += float(p.get("width", 0.0)) * float(p.get("height", 0.0))
