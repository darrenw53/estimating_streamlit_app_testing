...
def _to_inches(value: float, units: str) -> float:
    return float(value) / MM_PER_INCH if units == "Metric (mm)" else float(value)

def _from_inches(value: float, units: str) -> float:
    return float(value) * MM_PER_INCH if units == "Metric (mm)" else float(value)
...
APP_TITLE = "Estimating Calculator(V5-Test)"
...
def _init_state() -> None:
    ...
    st.session_state.setdefault("dxf_geom_wkt_map", {})
...
def _create_yield_image(sheet_layout: Dict[str, Any], scale: int = 5) -> Image.Image:
    ...
    Supports:
      - Rectangular parts (width/height)
      - Polygon parts via `part["poly"]` list of (x,y) points in inches
    """
    padding = 20
    ...
    for i, part in enumerate(sheet_layout.get("parts", [])):
        poly = part.get("poly")
        if poly and isinstance(poly, (list, tuple)) and len(poly) >= 3:
            pts = []
            for (px, py) in poly:
                pts.append((padding + int(float(px) * scale), padding + int(float(py) * scale)))
            draw.polygon(pts, fill=palette[i % len(palette)], outline="black")
            # Label near bbox
            try:
                xs = [p[0] for p in pts]
                ys = [p[1] for p in pts]
                ...
                bh = max(ys) - by0
                if bw > 40 and bh > 15:
                    draw.text((bx0 + 5, by0 + 5), str(part.get("part_name", "")), fill="black", font=font)
            except Exception:
                pass
        else:
            x0 = padding + int(float(part.get("x", 0.0)) * scale)
            y0 = padding + int(float(part.get("y", 0.0)) * scale)
            x1 = x0 + int(float(part.get("width", 0.0)) * scale)
            y1 = y0 + int(float(part.get("height", 0.0)) * scale)
            draw.rectangle([x0, y0, x1, y1], fill=palette[i % len(palette)], outline="black", width=1)
            if float(part.get("width", 0.0)) * scale > 40 and float(part.get("height", 0.0)) * scale > 15:
                draw.text((x0 + 5, y0 + 5), f"{float(part.get('width',0.0)):.1f}x{float(part.get('height',0.0)):.1f}", fill="black", font=font)
...
def page_plate() -> None:
        ...
        if uploaded:
            ...
            for f in uploaded:
                try:
                    ...
                    part = geom.part
                    geom_key = f"{f.name}::{part.part_name}"
                    st.session_state["dxf_geom_wkt_map"][geom_key] = getattr(geom.outer_poly_in, "wkt", "")
                    try:
            ...
            if detected_rows:
                ...
                edited = st.data_editor(
                    ...
                    num_rows="fixed",
                    column_config={
                        "Geom Key": st.column_config.TextColumn("Geom Key", disabled=True),
                ...
                if add_all:
                    ...
                    for _, r in edited.iterrows():
                        try:
                            ...
                            geom_key = str(r.get("Geom Key",""))
                            profile_wkt = ""
                            try:
                                profile_wkt = str((st.session_state.get("dxf_geom_wkt_map", {}) or {}).get(geom_key, "") or "")
                            except Exception:
                                profile_wkt = ""
    ...
    plate_input_units = st.radio(
        "Manual plate entry units",
        options=["Imperial (in)", "Metric (mm)"],
        horizontal=True,
        key="plate_input_units",
    )
    ...
    with c2:
        width_input = st.number_input(
            "Width (mm)" if plate_input_units == "Metric (mm)" else "Width (in)",
            min_value=0.0,
            value=_from_inches(float(st.session_state.get("plate_w", 0.0)), plate_input_units),
            step=1.0 if plate_input_units == "Metric (mm)" else 0.25,
            key="plate_w_input",
        )
        length_input = st.number_input(
            "Length (mm)" if plate_input_units == "Metric (mm)" else "Length (in)",
            min_value=0.0,
            value=_from_inches(float(st.session_state.get("plate_l", 0.0)), plate_input_units),
            step=1.0 if plate_input_units == "Metric (mm)" else 0.25,
            key="plate_l_input",
        )
        width = _to_inches(width_input, plate_input_units)
        length = _to_inches(length_input, plate_input_units)
    ...
    if add:
        ...
        st.session_state["plate_w"] = float(width)
        st.session_state["plate_l"] = float(length)
...
def page_structural() -> None:
    ...
    structural_input_units = st.radio(
        "Structural length entry units",
        options=["Imperial (in)", "Metric (mm)"],
        horizontal=True,
        key="struct_input_units",
    )
    ...
    with c2:
        ...
        length_input = st.number_input(
            "Length (mm)" if structural_input_units == "Metric (mm)" else "Length (in)",
            min_value=0.0,
            value=0.0,
            step=1.0,
        )
        length_in = _to_inches(length_input, structural_input_units)
    ...
    if add:
        ...
        part = {
            ...
            "Part Name": part_name,
            "Quantity": int(quantity),
            "Input Units": structural_input_units,
...
def page_summary() -> None:
    ...
    with st.expander("Plate nesting optimization (sheet cutting)", expanded=False):
        st.caption(
            ...
            "DXF parts will nest by their **true outer profile** when available. Manually entered plates nest as rectangles."
        )
                            ...
                            required_parts.append(
                                {
                                    ...
                                    "profile_wkt": str(r.get("Profile WKT","") or ""),
                                }
                            )
                        except Exception:
                            continue

                    sol = logic.optimize_plate_nesting_mixed(
                        required_parts,
                        stock_sizes,
