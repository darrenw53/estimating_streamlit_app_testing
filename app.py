...
def _init_state() -> None:
    ...
    st.session_state.setdefault("plate_stock_df", None)
...
def page_summary() -> None:
    ...
    with st.expander("Plate nesting optimization (sheet cutting)", expanded=False):
        ...
        if not plate_rows:
            ...
            groups = {}
            for r in plate_rows:
                key = (str(r.get("Material", "")), float(r.get("Thickness (in)", 0.0) or 0.0))
            ...
            stock_df = st.data_editor(
                st.session_state.get("plate_stock_df", default_stock),
                use_container_width=True,
                num_rows="dynamic",
                key="plate_stock_editor",
                column_config={
                    "Width (in)": st.column_config.NumberColumn("Width (in)", min_value=1.0, step=1.0, required=True),
                    "Height (in)": st.column_config.NumberColumn("Height (in)", min_value=1.0, step=1.0, required=True),
